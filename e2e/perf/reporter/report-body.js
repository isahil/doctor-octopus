...
export function generate_summary_metrics(
  ...
  const standard_artillery_prefixes = [
    "vusers.",
    "http.",
    "browser.",
    "errors.",
        ...
        <div class="section-container">
          <table>
            ...
            <tbody>
              ${status_codes
                .map(({ code, count }) => {
                  const percentage = ((count / http_responses) * 100).toFixed(
                    1,
                  );
...
export function generate_response_time_percentiles(response_time_summary) {
  if (
    !response_time_summary ||
    Object.keys(response_time_summary).length === 0
  ) {
    return "";
  }
  return `
  ...
  const intermediate_rows = intermediate
    .map((period) => {
      ...
      sorted_metrics.forEach((metric) => {
        ...
        const display =
          typeof value === "number" ? (value / 1000).toFixed(2) : "N/A";
        row += `
...
export function generate_browser_metrics(aggregate) {
  ...
  const browser_http_requests =
    aggregate.counters["browser.http_requests"] || 0;
  const browser_traces_collected =
    aggregate.counters["browser.traces_collected"] || 0;
  const browser_traces_discarded =
    aggregate.counters["browser.traces_discarded"] || 0;
  ...
  if (
    browser_http_requests === 0 &&
    Object.keys(browser_page_codes).length === 0
  ) {

...
const { AWS_SDET_BUCKET_NAME, ENVIRONMENT: environment, APP } = process.env;
const app = APP ?? "loan";
...
const json_report = `${local_test_reports_dir}/report.json`; // Read the report.json file to upload
...
const upload_report = async (code) => {
	...
	if(!CI) return process.exit(code ?? 1);
	await upload_directory(AWS_SDET_BUCKET_NAME, local_test_reports_dir, s3_test_reports_dir);
	process.exit(code ?? 1);
};

const command = `npx playwright test --project=${test_script_name}`
spawn_child_process(command, upload_report);

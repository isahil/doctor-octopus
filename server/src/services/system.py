...
report_cards_path = os.path.join("./", test_reports_dir)  # "./test-reports"


def local_report_directories():
    """get all local report directories"""
    card_directories = os.listdir(report_cards_path)
    logger.info(f"Total reports found on local: {len(card_directories)}")
    # logger.info(f"Local report directories: {card_directories}")
    return card_directories


def format_local_dir_filter_data(card_dir):
    ...
    return {
        "object_name": card_dir,
        "day": card_dir,
        # "protocol": path_parts[4],
        # "environment": path_parts[3],
        # "app": path_parts[2],
        # "file_type": "json",
        "local_root_dir": card_dir,
    }


def get_all_local_cards(expected_filter: dict) -> dict:
    ...
    for received_card_filter in formatted_cards_filter_data:
        day = int(expected_filter.get("day", ""))
        if not less_or_eqaul_to_date_time(received_card_filter["day"], day):
            continue

        report_dir_date = received_card_filter["day"]
        if report_dir_date not in final_cards_pool:
            final_cards_pool[report_dir_date] = {
                "filter_data": received_card_filter,
                "html_report": f"{report_dir_date}/index.html",
                "json_report": {},
                "root_dir": received_card_filter["local_root_dir"],
            }
...
async def process_card(card_tuple) -> dict | None:
    ...
    try:
        ...
        local_report_dir_path = os.path.join(report_cards_path, card_value["root_dir"])
...
def get_a_local_card_html_report(html) -> str:
    ...
    html_file_path = os.path.join(report_cards_path, html)
...
async def wait_for_local_report_to_be_ready(root_dir):
    try:
        report_dir = os.path.join(report_cards_path, root_dir)
...
def cleanup_old_test_report_directories(max_dirs=10):
    ...
    try:
        ...
        for item in os.listdir(report_cards_path):
            item_path = os.path.join(report_cards_path, item)

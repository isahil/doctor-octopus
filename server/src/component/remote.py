...
reports_dir = os.path.join(
    local_dir, test_reports_dir
)  # Full path to the local test reports directory
...
def get_all_s3_cards(filter: int) -> list:
    ...
    sorted_test_results = sorted(
        test_results, key=lambda x: x["json_report"]["stats"]["startTime"], reverse=True
    )

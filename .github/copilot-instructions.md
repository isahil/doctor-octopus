...
5. Terminal displays live logs using XTerm.js + SocketIO
6. On completion, test reports generated by the test runner are uploaded to AWS S3
7. Notification service's event listener will then cache in Redis for fast Cards page loading
8. User views HTML reports by clicking "View" button, which will take the user to the statically hosted report server

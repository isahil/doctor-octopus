...
const test_protocol_value = (test_suite) =>
  test_suite
    .split(delimiter(test_suite))[0]
    .replace(".yml", "")
    .replace(".yaml", "") || "na";

const reports_dir = get_test_reports_dir().split("/").pop() || "na";

export const upload_report = async (
  code,
  { test_suite, full_test_reports_dir },
) => {
  ...
  try {
    ...
    console.error(
      `Error requesting Doctor Octopus to download: ${err.message}`,
    );
  }
...
export const add_stats_to_json = ({ test_suite, json_report_path, runner }) => {
  ...
  if (is_ci) {
    ...
    const run_url =
      run_id && repo ? `${server}/${repo}/actions/runs/${run_id}` : "na";

...
class Cards:
    ...
    def download_missing_cards(self, expected_filter_data: dict, rate_limit_wait=rate_limit_wait_time) -> None:
        ...
        envs_to_check = (
            [expected_filter_data.get("environment")] if expected_filter_data.get("environment") else test_environments
        )
        ...
        missing_cards_in_envs = sum(
            cards_missing_per_environment, []
        ...
        logger.info(
            f"Missing cards to download: {len(missing_cards_in_envs)} total in {total_batches} batches -> {missing_cards_in_envs}"
        )

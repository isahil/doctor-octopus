...
test.describe("Doctor Octopus Reports Page UI Test", () => {
  test("Cards Component UI Validation", async ({ page }) => {
    ...
    await step("Verify Filters visibility", async () => {
      ...
      await expect
        .soft(day_filter, "Day filter should be visible")
        .toBeVisible();

      const product_filter = cards.filter_locator("product");
      await expect
        .soft(product_filter, "Product filter should be visible")
        .toBeVisible();

      const protocol_filter = cards.filter_locator("protocol");
      await expect
        .soft(protocol_filter, "Protocol filter should be visible")
        .toBeVisible();
    });
    ...
    await step("Apply Filters and verify results", async () => {
      ...
      expect
        .soft(
          card_count,
          "There should be some cards displayed after applying filters",
        )
        .toBeGreaterThan(0);
    });
  });

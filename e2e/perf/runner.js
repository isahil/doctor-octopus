...
export const execute_runner = async ({ test_suite, environment, tags }) => {
  ...
  if (!test_suite || !environment) {
    throw new Error(
      `Required script arguments are missing! [${get_est_date_time()}]`,
    );
  }
  ...
  feature =
    test_suite.split("/").pop()?.replace(".yml", "").replace(".yaml", "") ||
    "na";
  ...
  const artillery_record = record
    ? ` --record --key ${artillery_cloud_api_key}`
    : "";
